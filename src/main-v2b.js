...
const BUILD = 'v2b — path-following printer, variable print times';
...
app.innerHTML = `
  ...
  <section id="hud">
    <div class="topline">
      <h1>World Printer Lab <span style="color:#00ff9d">v2b</span></h1>
    ...
    <div class="hud-body">
      ...
      <p class="note"><b>${BUILD}</b>. Nozzle follows visible toolpaths. Bigger objects take longer. Fresh filament glows.</p>
      ...
      <div class="row">
        ...
        <span class="pill">toolpath nozzle</span>
        <span class="pill">variable time</span>
        <span class="pill">fresh layer glow</span>
  ...
  <aside id="help">v2b: the machine prints first. Watch nozzle movement + glowing bead paths, then pick up and place.</aside>
`;
...
function makeTexture(kind, base, line, size = 256) {
  ...
  if (kind === 'layers') {
    for (let y = 0; y < size; y++) {
      ctx.fillStyle = Math.floor(y / 5) % 2 ? line : base;
  ...
  if (kind === 'wood') {
    ...
    for (let x = 0; x < size; x += 2) {
      const wave = Math.sin(x * 0.08) * 10 + Math.sin(x * 0.021) * 18;
      ctx.strokeStyle = x % 12 < 6 ? line : base;
      ctx.beginPath();
      ctx.moveTo(x, 0);
      ctx.bezierCurveTo(x + wave, 70, x - wave, 180, x + wave * 0.3, size);
      ctx.stroke();
    }
    ctx.globalAlpha = 1;
  }
  ...
  if (kind === 'metal') {
    for (let i = 0; i < 1400; i++) {
      const v = 95 + Math.random() * 120;
      ctx.strokeStyle = `rgba(${v},${v},${v},0.2)`;
      const y = Math.random() * size;
      ctx.beginPath();
      ctx.moveTo(Math.random() * size, y);
      ctx.lineTo(Math.random() * size, y + Math.random() * 7 - 3.5);
      ctx.stroke();
    }
  }
...
const maps = {
  greenLayer: makeTexture('layers', '#0ee798', '#08b16f'),
  orangeLayer: makeTexture('layers', '#ff9738', '#d7601f'),
  whiteLayer: makeTexture('layers', '#e6e0cb', '#bfb494'),
  wood: makeTexture('wood', '#8b5a32', '#5b331f'),
  metal: makeTexture('metal', '#89938e', '#39413e'),
  darkMetal: makeTexture('metal', '#2a3431', '#101514'),
  cloth: makeTexture('cloth', '#d66f50', '#ffe0aa')
};
...
const mat = {
  ...
  dark: new THREE.MeshStandardMaterial({ color: 0x17201e, map: maps.darkMetal, roughness: 0.38, metalness: 0.78 }),
  metal: new THREE.MeshStandardMaterial({ color: 0xa7afaa, map: maps.metal, roughness: 0.28, metalness: 0.86 }),
  rubber: new THREE.MeshStandardMaterial({ color: 0x050706, roughness: 0.82, metalness: 0.01 }),
  green: new THREE.MeshStandardMaterial({ color: 0x00e996, map: maps.greenLayer, roughness: 0.34, metalness: 0.02, emissive: 0x003020 }),
  orange: new THREE.MeshStandardMaterial({ color: 0xff8730, map: maps.orangeLayer, roughness: 0.36, metalness: 0.02, emissive: 0x2a1000 }),
  creamPrinted: new THREE.MeshStandardMaterial({ color: 0xe6d9bd, map: maps.whiteLayer, roughness: 0.42, metalness: 0.01, emissive: 0x0c0802 }),
  ...
  wood: new THREE.MeshStandardMaterial({ color: 0x9a683b, map: maps.wood, roughness: 0.62, metalness: 0.02 }),
  darkWood: new THREE.MeshStandardMaterial({ color: 0x4b2d1e, map: maps.wood, roughness: 0.68, metalness: 0.01 }),
  wall: new THREE.MeshStandardMaterial({ color: 0xcab17b, map: maps.whiteLayer, roughness: 0.72, metalness: 0.01 }),
  roof: new THREE.MeshStandardMaterial({ color: 0x943848, roughness: 0.48, metalness: 0.04 }),
  cloth: new THREE.MeshStandardMaterial({ color: 0xd96f51, map: maps.cloth, roughness: 0.82, metalness: 0.0 }),
  cream: new THREE.MeshStandardMaterial({ color: 0xf4d9a4, roughness: 0.68, metalness: 0.02 }),
...
function transparent(material, opacity) { const m = material.clone(); m.transparent = true; m.opacity = opacity; m.depthWrite = opacity > 0.65; return m; }
function setGhost(root, on) { root.traverse((c) => { if (!c.isMesh) return; c.userData.originalMaterial ||= c.material; c.material = on ? mat.ghost : c.userData.originalMaterial; }); }
function setReveal(root, opacity) { root.traverse((c) => { if (!c.isMesh) return; c.userData.originalMaterial ||= c.material; c.material = transparent(c.userData.originalMaterial, opacity); }); }
function restore(root) { root.traverse((c) => { if (c.isMesh && c.userData.originalMaterial) c.material = c.userData.originalMaterial; }); }
...
function label(text, color = '#00ff9d') {
  ...
  s.scale.set(3.8, 0.9, 1);
  return s;
}

function createPrinter() {
  const g = new THREE.Group();
  g.name = 'v2b Path Printer';
  ...
  const gantry = new THREE.Group(); gantry.name = 'moving gantry'; gantry.position.set(0,5.55,0.25);
  const beam = new THREE.Mesh(new THREE.BoxGeometry(5.85,0.3,0.45), mat.dark); gantry.add(beam);
  ...
  g.add(gantry);
  const carriage = new THREE.Group(); carriage.name = 'moving hotend'; carriage.position.set(0,5.1,0.25);
  ...
  const led = new THREE.PointLight(0x00ff9d, 1.6, 5); led.position.set(0,-0.92,0.1); carriage.add(led);
  ...
  const spoolBody = cyl(0.44,0.34,mat.green,40); spoolBody.rotation.z = Math.PI/2; spool.add(spoolBody);
  ...
  const status = new THREE.Mesh(new THREE.BoxGeometry(1.6,0.12,0.08), mat.freshGreen); status.position.set(0,6.25,-2.18); g.add(status);
  const sign = label('WORLD PRINTER v2b'); sign.position.set(0,6.52,-2.22); g.add(sign);
  g.userData = { carriage, gantry, spool, led, nozzleTipLocal: new THREE.Vector3(0,-1.13,0), idle: carriage.position.clone() };
  return shadow(g);
}
...
const recipes = [
  { id:'spiral', label:'Spiral', aliases:['spiral','twist','knot'], dims:[2.2,2.2,2.6], complexity:1.25, kind:'organic', create:createSpiral },
  { id:'creature', label:'Creature', aliases:['creature','robot','eyeball','monster','character'], dims:[2.2,2.2,1.8], complexity:1.35, kind:'organic', create:createCreature },
  { id:'cottage', label:'Cottage', aliases:['cottage','house','home','hut'], dims:[3.8,3.2,3.2], complexity:1.0, kind:'rect', create:createCottage },
  { id:'boat', label:'Boat', aliases:['boat','ship','sailboat'], dims:[3.7,1.8,2.7], complexity:1.0, kind:'hull', create:createBoat },
  { id:'tree', label:'Tree', aliases:['tree','forest','oak'], dims:[2.5,2.5,3.4], complexity:1.15, kind:'organic', create:createTree },
  { id:'bridge', label:'Bridge', aliases:['bridge','arch bridge','wood bridge'], dims:[4.4,2.8,2.0], complexity:0.95, kind:'rect', create:createBridge },
  { id:'cart', label:'Cart', aliases:['cart','wagon','carriage'], dims:[3.0,1.9,1.8], complexity:0.8, kind:'rect', create:createCart },
  { id:'stall', label:'Market Stall', aliases:['market','stall','shop','vendor'], dims:[3.4,2.2,3.1], complexity:1.08, kind:'rect', create:createStall },
  { id:'campfire', label:'Campfire', aliases:['campfire','fire','firepit'], dims:[1.4,1.4,1.2], complexity:0.55, kind:'organic', create:createCampfire }
];
...
function printDuration(recipe) { const [w,d,h]=recipe.dims; const volume=w*d*h; return Math.round((4200 + volume*210) * recipe.complexity); }
...
function pointOnPath(recipe, raw) {
  const [w,d,h]=recipe.dims;
  const layerCount = Math.round(24 + h*7 + recipe.complexity*8);
  const layerFloat = raw * layerCount;
  const layer = Math.floor(layerFloat);
  const t = layerFloat - layer;
  const y = 0.035 + h * (layer / layerCount) * 0.88;
  if (recipe.kind === 'organic') {
    const a = t * Math.PI * 2 + layer * 0.72;
    const pulse = 0.78 + 0.22 * Math.sin(layer * 0.41);
    const r = Math.min(w,d) * 0.24 * pulse;
    return new THREE.Vector3(Math.cos(a)*r, y, Math.sin(a)*r);
  }
  if (recipe.kind === 'hull') {
    const a = t * Math.PI * 2;
    const taper = 1 - (layer / layerCount) * 0.22;
    return new THREE.Vector3(Math.cos(a)*w*0.34*taper, y, Math.sin(a)*d*0.22*(0.75+0.25*Math.cos(a))*taper);
  }
  const w2=w*0.38*(1-(layer/layerCount)*0.05), d2=d*0.38*(1-(layer/layerCount)*0.05);
  const loop=t*5;
  if (loop < 1) return new THREE.Vector3(-w2 + loop*2*w2, y, -d2);
  if (loop < 2) return new THREE.Vector3(w2, y, -d2 + (loop-1)*2*d2);
  if (loop < 3) return new THREE.Vector3(w2 - (loop-2)*2*w2, y, d2);
  if (loop < 4) return new THREE.Vector3(-w2, y, d2 - (loop-3)*2*d2);
  const infill = (loop - 4);
  return new THREE.Vector3(-w2 + infill*2*w2, y, Math.sin(layer*0.9)*d2*0.55);
}

function nozzleLocalFromPrintPoint(p) { return new THREE.Vector3(p.x, 1.08 + p.y, p.z + 0.28); }
function segment(a,b,material=mat.freshGreen,r=0.026){ if(a.distanceTo(b)<0.01) return null; return new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3([a,b]),5,r,8),material); }
function dripLine(a,b){ const mid=a.clone().lerp(b,0.5); const length=a.distanceTo(b); const geo=new THREE.CylinderGeometry(0.018,0.018,Math.max(0.05,length),8); const mesh=new THREE.Mesh(geo,mat.freshGreen); mesh.position.copy(mid); mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0), b.clone().sub(a).normalize()); return mesh; }
...
async function startPrint(recipe){
  ...
  clearSelection();
  setPhase('printing');
  const duration=printDuration(recipe);
  setStatus(`Printing ${recipe.label}. Estimated print time: ${(duration/1000).toFixed(1)}s. Nozzle is following perimeter + infill paths.`);
  const obj=recipe.create(); obj.userData={label:recipe.label, recipeId:recipe.id, state:'printing'}; obj.position.copy(bedWorld()); obj.scale.setScalar(0.015); setReveal(obj,0.12); scene.add(obj); printedOnBed=obj;
  ...
  const t0=performance.now(); let prev=null, lastEmit=0, segCount=0;
  await new Promise(resolve=>{
    function step(now){
      const raw=clamp01((now-t0)/duration);
      const eased=1-Math.pow(1-raw,2.05);
      const p=pointOnPath(recipe,raw);
      const cLocal=nozzleLocalFromPrintPoint(p);
      carriage.position.copy(cLocal);
      gantry.position.y = 5.25 + p.y * 0.18;
      spool.rotation.x += 0.04;
      const bedP=bedWorld(p);
      const noz=nozzleWorld();
      liveBead.position.copy(bedP);
      if(liveThread) scene.remove(liveThread);
      liveThread = dripLine(noz, bedP);
      scene.add(liveThread);
      obj.position.copy(bedWorld()); obj.scale.setScalar(0.015+eased*0.985); setReveal(obj,0.12+eased*0.88);
      if(prev && now-lastEmit>42 && segCount<720){
        const a=bedWorld(prev), b=bedP;
        const s=segment(a,b, recipe.id==='creature'||recipe.id==='spiral'?mat.freshOrange:mat.freshGreen, recipe.kind==='organic'?0.03:0.024);
  ...
  restore(obj); obj.scale.setScalar(1); obj.position.copy(bedWorld()); obj.userData.state='printed-on-bed';
  setPhase('printed-on-bed'); setTarget(`${recipe.label} finished on printer bed`);
  setStatus(`${recipe.label} finished. It kept printed/layered material. Pick it up to make it a placement preview.`);
}
...
function placePreview(){ if(!carriedPreview){setStatus('No carried preview. Print something, then Pick Up Print first.');return;} const obj=carriedPreview; carriedPreview=null; setGhost(obj,false); restore(obj); obj.userData.id=++idCounter; obj.userData.state='placed'; placed.push(obj); setPhase('ready'); selectPlaced(obj); setStatus(`${obj.userData.label} placed as a solid v2b printed world object.`); }
...
runButton.addEventListener('click',()=>{ const r=parseCommand(commandInput.value); if(!r){setStatus('No recipe matched. Try spiral, creature, cottage, boat, tree, bridge, cart, stall, or campfire.');return;} startPrint(r); });
...
if(!SpeechRecognition){ voiceButton.disabled=true; voiceButton.title='Speech recognition not available in this browser.'; } else { const rec=new SpeechRecognition(); rec.lang='en-US'; rec.interimResults=false; rec.continuous=false; voiceButton.addEventListener('click',()=>{ setStatus('Listening. Say make a spiral, creature, boat, cottage...'); rec.start(); }); rec.onresult=(e)=>{ const text=e.results[0][0].transcript; commandInput.value=text; const r=parseCommand(text); if(r)startPrint(r); else setStatus(`Heard “${text}”, but no recipe matched yet.`); }; rec.onerror=(e)=>setStatus(`Voice error: ${e.error}. Type the command instead.`); }
...
setPhase('ready'); setTarget('none'); setStatus(`${BUILD}. Stable root is untouched. Test print times: campfire short, stall/cottage longer, spiral/creature complex.`); animate(performance.now());
